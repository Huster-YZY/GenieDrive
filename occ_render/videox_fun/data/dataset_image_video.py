...
class ImageVideoControlDataset(Dataset):
    def __init__(
        ...
        #additional support for multi-view videos loading
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image')=='video':
            ...
            pixel_values_mv = []
            control_pixel_values_mv = []
            clip_seg = []


            for i, cam_name in enumerate(self.cam_names):
                video_id = data_info[f'file_path_{cam_name}']
                if self.data_root is None:
                    video_dir = video_id
                else:
                    video_dir = os.path.join(self.data_root, video_id)
                ...
                with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
                    ...
                    if i==0: # to ensure the sampeld idxs are the same in multi-view
                        video_length = int(self.video_length_drop_end * len(video_reader))
                        clip_length = min(video_length, (min_sample_n_frames - 1) * self.video_sample_stride + 1)
                        start_idx   = random.randint(int(self.video_length_drop_start * video_length), video_length - clip_length) if video_length != clip_length else 0
                        batch_index = np.linspace(start_idx, start_idx + clip_length - 1, min_sample_n_frames, dtype=int)
                ...
                pixel_values_mv.append(pixel_values)

                # control_video_id = data_info['control_file_path']
                control_video_id = data_info[f'control_file_path_{cam_name}']
                ...
                control_pixel_values_mv.append(control_pixel_values)
            ...
            # print(pixel_values.shape, control_pixel_values.shape) #(37, 900, 1600, 3) (37, 450, 800, 3)
            # print(type(pixel_values_mv[0]), len(pixel_values_mv)) #np.array 6
            # exit(-1)

            pixel_values_mv = np.stack(pixel_values_mv, axis=0)
            control_pixel_values_mv = np.stack(control_pixel_values_mv, axis=0)

            return pixel_values_mv, control_pixel_values_mv, subject_image, control_camera_values, text, "video"
        else:
            image_path, text = data_info['file_path'], data_info['text']
            if self.data_root is not None:
                image_path = os.path.join(self.data_root, image_path)
