...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=12, imageio_backend=True, color_transfer_post_process=False, save_per_frame = False):
    videos = rearrange(videos, "b c t h w -> t b c h w")
    
    # print(videos.shape) #torch.Size([37, 6, 3, 512, 256])
    # exit(-1)
    ...
    if save_per_frame:
        ...
        for idx, frame in enumerate(outputs):
            frame.save(os.path.join(save_dir, f'{idx:06d}.png'))
...
def read_video(input_video_path, sample_size, fps, read_frames_num = None):
    ...
    while True:
        ...
        if frame_count % frame_skip == 0:
            ...
            if read_frames_num is not None and len(input_video) == read_frames_num:
                break
