...
def rope_apply(x, grid_sizes, freqs):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        output.append(x_i)
    return torch.stack(output).to(x.dtype) #[6, 5632, 12, 128]

@amp.autocast(enabled=False)
@torch.compiler.disable()
def rope_apply_cross_view(x, grid_sizes, freqs):
    x = torch.view_as_complex(x.to(torch.float32).unflatten(-1, (-1, 2)))
    freqs = freqs.unsqueeze(1).unsqueeze(0).to(x.device)
    x = torch.view_as_real(x * freqs).flatten(3)
    return x


def rope_apply_qk(q, k, grid_sizes, freqs, cross_view):
    if cross_view:
        q = rope_apply_cross_view(q, grid_sizes, freqs)
        k = rope_apply_cross_view(k, grid_sizes, freqs)
    else:
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        x = cross_attn_ffn(x.to(dtype), context, context_lens, e)
        ...
        if self.use_cross_view:
            ...
            bs = x.shape[0]
            x = rearrange(x, 'b v (t h w) d -> (b t h) (v w) d', h=16, w=32)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        x = [u.flatten(2).transpose(1, 2) for u in x] # [1, 5120, 1536]
