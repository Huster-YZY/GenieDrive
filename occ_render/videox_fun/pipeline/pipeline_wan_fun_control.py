...
class WanFunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        #num_videos_per_prompt = 1 #todo: remove this line of code
        ...
        # print(batch_size)
        # print(type(prompt_embeds), len(prompt_embeds))
        # exit(-1)

        # duplicate prompt 6 times to match multi-view input
        # print("Duplicating!!!")
        negative_prompt_embeds = negative_prompt_embeds * 6
        prompt_embeds = prompt_embeds * 6
        ...
        # print(clip_image.shape, clip_context.shape) #torch.Size([3, 512, 512]) torch.Size([1, 257, 1280])
        # exit(-1)
        
        if clip_image is not None:
            #todo: support image list
            if isinstance(clip_image, list):
                clip_image = torch.stack([TF.to_tensor(clip_image_view).sub_(0.5).div_(0.5).to(device, weight_dtype) for clip_image_view in clip_image], dim=1)
                clip_context = self.clip_image_encoder([clip_image])
                # print(clip_image.shape, clip_context.shape) #torch.Size([3, 6, 900, 1600]) torch.Size([6, 257, 1280])
                # exit(-1)
            else:
                # clip_image = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(device, weight_dtype) 
                # clip_context = self.clip_image_encoder([clip_image[:, None, :, :]])

                clip_image = clip_image.sub_(0.5).div_(0.5).to(device, weight_dtype) 
                clip_context = self.clip_image_encoder([clip_image])
        else:
            clip_image = Image.new("RGB", (512, 512), color=(0, 0, 0))  
            clip_image = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(device, weight_dtype) 
        ...
        return WanPipelineOutput(videos=video, flows=latents_list)
