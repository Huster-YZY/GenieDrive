...
parser.add_argument("--save_path", type=str, default="samples/evaluation")
parser.add_argument("--sample_condition_path", type=str, required=True)
...
for cam_name in cam_names:
    video_path = os.path.join(args.sample_condition_path, cam_name + '.mp4')
    img_path = os.path.join(args.sample_condition_path, cam_name + '.jpg')
    assert os.path.exists(video_path) and os.path.exists(img_path)
...
with torch.no_grad():
    ...
    if ref_images_mv_paths is not None:
    ...
    if control_camera_txt is not None:
        ...
        input_video = torch.cat([get_video_to_video_latent(control_video, video_length=video_length, sample_size=sample_size, fps=fps, ref_image=None)[0] for control_video in control_videos_mv_paths], dim=0)
    ...
    def save_results(view_by_view = False):
        ...
        if video_length == 1:
            ...
            if not view_by_view:
                ...
                for i, cam_name in enumerate(cam_names):
                    view_path = os.path.join(save_path, f'{os.path.basename(args.sample_condition_path)}', f'{cam_name}.mp4')
                    os.makedirs(os.path.dirname(view_path), exist_ok=True)
                    save_videos_grid(sample[i:i+1], view_path, fps=fps, n_rows = 1)

    save_results(view_by_view = True)
