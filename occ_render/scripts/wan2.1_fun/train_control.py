...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="semantic-control-fine-tune",
        help=(
...
def main():
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            v, f, h, w, c      = np.shape(pixel_value)
            if data_type == 'image':
                ...
                if args.random_hw_adapt:
                    if args.training_with_video_token_length:
                        local_min_size = np.min(np.array([np.mean(np.array([np.shape(example["pixel_values"])[2], np.shape(example["pixel_values"])[3]])) for example in examples]))
            ...
            for example in examples:
                # To 0~1
                # pixel_values = torch.from_numpy(example["pixel_values"]).permute(0, 3, 1, 2).contiguous() #f h w c --> f c h w
                ...
                pixel_values = torch.from_numpy(example["pixel_values"]).flatten(0,1).permute(0, 3, 1, 2).contiguous() #v f h w c --> (vf) c h w
                pixel_values = pixel_values / 255.

                control_pixel_values = torch.from_numpy(example["control_pixel_values"]).flatten(0,1).permute(0, 3, 1, 2).contiguous()
                control_pixel_values = control_pixel_values / 255.
                ...
                new_examples["pixel_values"].append(transform(pixel_values).unflatten(0, (v, -1)))
                new_examples["control_pixel_values"].append(transform(control_pixel_values).unflatten(0, (v, -1)))
                ...
                if args.train_mode != "control":
                    ...
                    ref_pixel_values = new_examples["pixel_values"][-1][:, clip_index:clip_index+1] #unsqueeze a seq dim to match whole video sequence
                    ...
                    clip_pixel_values = new_examples["pixel_values"][-1][:, clip_index].permute(0, 2, 3, 1).contiguous()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                pixel_values = rearrange(pixel_values[0], "b f c h w -> b c f h w")
                control_pixel_values = rearrange(control_pixel_values[0], "b f c h w -> b c f h w")

                os.makedirs(os.path.join(args.output_dir, "sanity_check"), exist_ok=True)

                gif_name = f'{global_step}-mv'
                save_videos_grid(pixel_values, f"{args.output_dir}/sanity_check/{gif_name[:10]}.gif", rescale=True, n_rows = 3)
                save_videos_grid(control_pixel_values, f"{args.output_dir}/sanity_check/{gif_name[:10]}_control.gif", rescale=True, n_rows = 3)
                
                if args.train_mode != "control":
                    ref_pixel_values = batch["ref_pixel_values"].cpu()
                    ref_pixel_values = rearrange(ref_pixel_values[0], "b f c h w -> b c f h w")

                    gif_name = f'{global_step}-mv'
                    save_videos_grid(ref_pixel_values, f"{args.output_dir}/sanity_check/{gif_name[:10]}_ref.gif", rescale=True, n_rows = 3)
            ...
            with accelerator.accumulate(transformer3d):
                ...
                # flatten the batch_size and multi_view_dim
                pixel_values = pixel_values.flatten(0, 1)
                control_pixel_values = control_pixel_values.flatten(0, 1)
                ...
                if args.train_mode != "control":
                    ref_pixel_values = batch["ref_pixel_values"].to(weight_dtype).flatten(0, 1)
                    clip_pixel_values = batch["clip_pixel_values"].flatten(0, 1)
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "control":
                        ...
                        for bs_index in range(ref_latents.size()[0]):
                            ...
                            if clip_idx[bs_index//6] != 0 or (zero_init_ref_latents_conv_in and latents.size()[1] != 1):
                            ...
                            if args.add_full_ref_image_in_self_attention:
                                ...
                                if clip_idx[bs_index//6] == 0 or zero_init_full_ref_conv_in:
                ...
                if not args.uniform_sampling:
                    ...
                    indices = idx_sampling(1, generator=torch_rng, device=latents.device).repeat(bsz) #@zhenya: we assume effect bsz=1
