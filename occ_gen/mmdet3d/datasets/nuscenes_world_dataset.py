...
class NuScenesWorldDataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        # probe_scene_name = info['occ_path'].split('/')[-2]
        # if probe_scene_name == "scene-0476":
    ...
    def load_future_frame_info(self, index, info):
        ...
        curr_to_prev_ego_rt_cache = [] #@zhenya: support for end-to-end training
        ...
        for i in range(self.load_future_frame_number):
            ...
            if future_info['prev'] == last_future_info['token']:  # check the future frame is in the same sequence
                ...
                curr_to_prev_ego_rt_cache.append(torch.FloatTensor(nuscenes_get_rt_matrix(self.data_infos[future_index], self.data_infos[future_index - 1], "ego", "ego")))
            else:
                ...
                ego2lidar = transform_matrix(last_future_info['lidar2ego_translation'],
                                             ...
                                             inverse=True)
                ego_to_lidar.append(ego2lidar)
                curr_to_prev_ego_rt_cache.append(torch.FloatTensor(nuscenes_get_rt_matrix(self.data_infos[last_future_index], self.data_infos[last_future_index - 1], "ego", "ego")))

                valid_frame[i:] = False

        # print("I am here")
        # print(curr_to_prev_ego_rt_cache)
        # exit(-1)
        ...
        output_dict = dict(
            ...
            curr_to_prev_ego_rt_cache = np.array(curr_to_prev_ego_rt_cache)
        )
        return output_dict
