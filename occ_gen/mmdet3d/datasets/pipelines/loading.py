...
class LoadStreamLatentToken(object):
    ...
    def __call__(self, results):
        ...
        #load previous frame
        previous_tokens = []
        previous_frame = len(results['previous_occ_path'])
        for frame_idx in range(previous_frame):
            previous_sample_idx = results['previous_occ_path'][frame_idx].split('/')[-1]
            if self.dataset_type == 'waymo':
                scene_name = str(scene_name).zfill(3)
                pre_occ_index = results['previous_occ_path'][frame_idx].split('/')[-1].split('.')[0]
                latent_token_path = os.path.join(self.data_path, scene_name, f'{pre_occ_index}.npz')
            else:
                latent_token_path = os.path.join(self.data_path, str(scene_name), f'{previous_sample_idx}.npz')
            latent_token = np.load(latent_token_path)
            previous_token = latent_token['token']
            previous_tokens.append(previous_token)
            
        previous_tokens = np.stack(previous_tokens)

        input_token = np.concatenate([previous_tokens, token, future_tokens], axis=0)
