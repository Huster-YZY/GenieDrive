...
class MHA(nn.Module):
    def __init__(self, embed_dims, head):
        super(MHA, self).__init__()
        self.attn = nn.MultiheadAttention(embed_dims, head, batch_first=True)
...
class EE_Former(BaseModule):
    ...
    def __init__(self,
                 ...
                 task_mode='generate',
                 **kwargs):
        super(EE_Former, self).__init__(**kwargs)
        ...
        #add high frequency position encoding
        self.plan_freq_dim = 8
        plan_embed_dim = self.plan_freq_dim * 12

        self.plan_embed = nn.Sequential(
            nn.Linear(plan_embed_dim, embed_dims),
            nn.ReLU(True),
            nn.Linear(embed_dims, embed_dims)
        )
        ...
        self.pose_attn = MHA(embed_dims, 4)
        self.init_weights()
    ...
    def forward(
        ...
        bev_w, bev_h = [w], [h]
        for i in range(len(self.intra_encoder.layers)): #3->5
            bev_w.append(w)
            bev_h.append(h)
        ...
        if self.ego_mode == 1:
            ...
            plan_output = self.plan_translation_output(plan_embed).reshape(bs, -1, 6)
        ...
        curr_to_futu_info_embed = frequency_embedding(curr_to_futu_info, dim= self.plan_freq_dim)
        curr_to_futu_info_embed = self.plan_embed(curr_to_futu_info_embed).unsqueeze(1)
        bev_embed = self.pose_attn(bev_embed, curr_to_futu_info_embed, curr_to_futu_info_embed)
        ...
        # @zhenya: w h need to be switched for triplane rep.
