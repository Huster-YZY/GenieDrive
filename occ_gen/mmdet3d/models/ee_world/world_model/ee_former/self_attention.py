...
class SelfAttention(BaseModule):
    ...
    def __init__(self,
        ...
        if self.plan_aware:
            self.plan_injection_attn = nn.MultiheadAttention(embed_dims, num_heads=4, batch_first=True)
            self.plan_linear = nn.Sequential(
    ...
    def forward(self,
        ...
        if self.plan_aware:
            plan_attn_identity = query
            query = plan_attn_identity + self.plan_linear(self.plan_injection_attn(query, plan_queries, plan_queries)[0])
        ...
        # print(spatial_shapes[:,0], spatial_shapes[:, 1])
        # print((spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum())
        # print(num_value)
        # exit(-1)
