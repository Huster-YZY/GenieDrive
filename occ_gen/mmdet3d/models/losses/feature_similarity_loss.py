...
class FeatSimLoss(nn.Module):
    ...
    def forward(self, pred, targ, valid, sample_weight=None):
        ...
        # print(valid_pred.shape, valid_targ.shape)
        # probe_loss = torch.mean((valid_pred - valid_targ) ** 2, dim=(0, 1))
        # # print(probe_loss.shape)
        # xy_loss = torch.mean(probe_loss[:, :50])
        # xz_loss = torch.mean(probe_loss[:, 50:54])
        # yz_loss = torch.mean(probe_loss[:, 54:58])
        # print(xy_loss.item(), xz_loss.item(), yz_loss.item())
