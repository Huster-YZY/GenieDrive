...
def softmax_focal_loss(pred,
    ...
    loss = _softmax_focal_loss(pred.contiguous(), target.contiguous(), gamma,
                               alpha, None, 'none')
    loss = loss.unsqueeze(1) #to match the size of weight
        ...
        if self.use_sigmoid:
            if self.activated:
                ...
                if torch.cuda.is_available() and pred.is_cuda:
                    # calculate_loss_func = sigmoid_focal_loss  #@zhenya: replace sigmoid with softmax
